...
// MARK: - Device identity (wire types — must stay byte-identical to the macOS app)
...
/// Wire type — field names must match the macOS app exactly.
...
/// Wire type — case names must match the macOS app exactly.
...
// MARK: - Peers (local display models, not sent over the wire)
...
// MARK: - File transfers (local display models)
